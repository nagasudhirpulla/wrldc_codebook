from src.typeDefs.realTimeOutage import IRealTimeOutage
from typing import List
import cx_Oracle
from src.app.utils.getTimeDeltaFromDbStr import getTimeDeltaFromDbStr


def getLatestOutages(pwcDbConnStr: str) -> List[IRealTimeOutage]:
    fetchSql = """SELECT
        rto.id,
        rto.entity_id,
        rto.element_id,
    ...
    targetColumns = ['ID', 'ENTITY_ID', 'ELEMENT_ID', 'ELEMENTTYPE',
                     ...
                     'REVIVED_DATE', 'REVIVED_TIME', 'REVIVAL_REMARKS']
    outages: List[IRealTimeOutage] = []
    colNames = []
    dbRows = []
    dbConn = None
    dbCur = None
    try:
        ...
        dbRows = []
        print('Error while getting latest real time outages from pwc db')
        print(err)
    finally:
    ...
    for row in dbRows:
        ...
        # derive revial time
        revivalDt = None
        revTimeStr = row[colNames.index("REVIVED_TIME")]
        if not revTimeStr == None:
            revTimeDelta = getTimeDeltaFromDbStr(outTimeStr)
            revivalDt = row[colNames.index("REVIVED_DATE")]
            revivalDt = revivalDt.replace(
                hour=0, minute=0, second=0, microsecond=0)
            # add time to out date to get timestamp
            revivalDt += revTimeDelta

        outage: IRealTimeOutage = {
            ...
            "revivalDt": revivalDt,
            "revivalRemarks": row[colNames.index("REVIVAL_REMARKS")]
        }
