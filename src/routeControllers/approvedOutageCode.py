...
approvedOutageCodePage = Blueprint('approvedOutageCode', __name__,
                                   template_folder='templates')


class CreateApprovedOutageCodeForm(Form):
    code = StringField(
...
@approvedOutageCodePage.route('/create', methods=['GET', 'POST'])
@role_required('code_book_editor')
def create():
    form = CreateApprovedOutageCodeForm(request.form)
    appConf = getConfig()
    ...
    return render_template('approvedOutageCode/create.html.j2', form=form, data={"oTags": oTags, "oTypes": oTypes})
